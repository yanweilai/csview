  ...
  <div>
    <div class="i-layout-page-header">
      <PageHeader class="product_tabs" hidden-breadcrumb>
        <div slot="title">
          <router-link :to="{ path: `${roterPre}/marketing/activity_frame` }">
          ...
          <span
              v-text="$route.params.id !=0 ? '编辑活动边框' : '添加活动边框'"
              class="mr20 ml16"
          ></span>
        </div>
      </PageHeader>
      ...
      this.$refs[name].validate((valid) => {
        if (valid) {
          ...
          activityFrameSave(this.$route.params.id,this.formValidate).then(res=>{
            this.$router.push({ path: `${this.roterPre}/marketing/activity_frame` });
