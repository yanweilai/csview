  ...
  <div>
    <div class="i-layout-page-header">
      <PageHeader class="product_tabs" hidden-breadcrumb>
        <div slot="title">
          <router-link :to="{ path: `${roterPre}/marketing/activity_background` }">
          ...
          <span
              v-text="$route.params.id !=0 ? '编辑活动背景' : '添加活动背景'"
              class="mr20 ml16"
          ></span>
        </div>
      </PageHeader>
              ...
              <Row>
                <Col span="24">
                  <div v-if="formValidate.image" class="upload-list">
                    <div class="upload-item" :class="formValidate.image?'on':''">
...
import { activityBackgroundSave,activityBackgroundInfo } from "@/api/marketing";
export default {
  components: {
    uploadPictures,
    storeLabelList,
  ...
  methods: {
    ...
    //获取详情
    getInfo(){
      activityBackgroundInfo(this.$route.params.id).then(res=>{
    ...
    handleSubmit(name){
      this.$refs[name].validate((valid) => {
        if (valid) {
          ...
          activityBackgroundSave(this.$route.params.id,this.formValidate).then(res=>{
            this.$router.push({ path: `${this.roterPre}/marketing/activity_background` });
...
.ivu-btn[data-v-7bfea95a]:only-child{
