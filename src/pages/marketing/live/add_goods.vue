<template>
  <div>
    <PageHeader class="product_tabs" :title="$route.meta.title" hidden-breadcrumb>
      <div slot="title">
        <router-link :to="{path:`${roterPre}/marketing/live/live_goods`}">
    ...
    <Modal v-model="modals" title="商品列表"  class="paymentFooter" scrollable width="900" :footer-hide="true">
      <goods-list ref="goodslist"  @getProductId="getProductId" v-if="modals" :ischeckbox="true" :isLive="true"></goods-list>
    </Modal>
  </div>
</template>
  ...
  data(){
    return {
      ...
      columns1:[
        ...
        {"key": "price",  "title": "直播售价",minWidth:100,
          render: (h, params) => {
            return h('Input', {
              props: {
                type: 'number',
        ...
        {"key": "cost_price",  "title": "直播原价",minWidth:100,
          render: (h, params) => {
            return h('Input', {
              props: {
                type: 'number',
  ...
  methods:{
    ...
    bindSub(){
      liveGoodsAdd({
        ...
        setTimeout(() => {
          this.$router.push({ path: `${this.roterPre}/marketing/live/live_goods` });
        }, 500);
      }).catch(error=>{
        this.$Message.error(error.msg)
      })
