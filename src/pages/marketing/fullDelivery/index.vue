    ...
    <div>
        ...
        <div class="new_card_pd">
			...
			<Card :bordered="false" dis-hover class="ivu-mt">
				<!-- 操作 -->
			<Button type="primary" @click="add">添加满送活动</Button>
			<!-- 满送活动-表格 -->
            <Table :columns="columns1" :data="list" ref="table" class="ivu-mt"
                ...
                <template slot-scope="{ row, index }" slot="action">
                    ...
                    <a @click="del(row,'删除满送活动',index)">删除</a>
                </template>
            </Table>
    ...
    export default {
        ...
        methods:{
					  ...
					  onchangeIsShow (row) {
						...
						// 添加
						add () {
						    this.$router.push({ path: this.roterPre + "/marketing/discount/add_give/" + 0 });
						},
						discountSearchs(){
							this.discountFrom.page = 1;
							this.list = [];
            ...
            getList () {
                this.loading = true;
                discountList(4,this.discountFrom).then(res => {
                    let data = res.data;
                    this.list = data.list;
            ...
            //修改
            edit(id){
							  this.$router.push({ path: this.roterPre + "/marketing/discount/add_give/" + id });
            }
        }
    }
</script>
